...
import {
  createParticipantApiClient,
  getParticipantApiConfigFromPublicEnv,
  type ParticipantApiClient,
} from "../participant/api-client";
import {
  describeKakaoCallbackResult,
  describeSocialLoginAvailability,
  getSocialLoginConfig,
  type KakaoCallbackResult,
  type SocialLoginConfig,
} from "../auth/social-login-config";
import {
  defaultKakaoAuthClient,
  type KakaoAuthClient,
} from "../auth/kakao-auth-client";
...
const defaultParticipantApi = createParticipantApiClient(
  getParticipantApiConfigFromPublicEnv(),
);
const happickleLogo = require("../../assets/happickle_logo.png");
const defaultTournamentId =
  sandboxParticipantSession.featuredTournament.tournamentId;
...
type ParticipantStoreState = {
  ...
  activePaymentOrder: PaymentOrderResponse | null;
  activeRefundRequest: RefundRequest | null;
  participantGames: ParticipantGame[];
  apiMode: "mock" | "api" | "fallback";
  routeStatus: Partial<
    Record<
      "support" | "notifications" | "mypage" | "games" | "tournamentDetail",
      "loading" | "ready" | "fallback"
    >
  >;
  supportInquirySubmission: "idle" | "submitting" | "submitted" | "fallback";
  paymentOrderSubmission: "idle" | "submitting" | "submitted" | "failed";
  refundSubmission: "idle" | "submitting" | "submitted" | "failed";
};
...
type CompleteKakaoAdditionalInfo = (input: {
  continuationToken: string;
  email: string;
  displayName: string;
  phone?: string;
}) => Promise<Extract<KakaoCallbackResult, { action: "login" | "signup" }>>;

let persistedKakaoDevSession: KakaoDevSessionSnapshot | null = null;

const fallbackSupportCenter: SupportCenterResponse = {
  policyCopy: describeSupportRefundPolicyCopy({
...
const fallbackNotifications: ParticipantNotification[] = [
  {
    notificationId: "notification_local_deadline",
    participantId: sandboxParticipantSession.profile.participantId,
...
const initialParticipantState = (
  participantApi: ParticipantApiClient,
  ...
  paymentOrderSubmission: "idle",
  refundSubmission: "idle",
});
...
function patchRouteStatus(
  route: keyof ParticipantStoreState["routeStatus"],
  status: NonNullable<
    ParticipantStoreState["routeStatus"][keyof ParticipantStoreState["routeStatus"]]
  >,
) {
  patchParticipantState({
    routeStatus: { ...participantState.routeStatus, [route]: status },
  });
}
...
export function resetParticipantFlow(
...
function isKakaoDevSessionSuccess(
  result: KakaoCallbackResult | null | undefined,
): result is Extract<KakaoCallbackResult, { action: "login" | "signup" }> {
  return Boolean(
    result &&
    "action" in result &&
    (result.action === "login" || result.action === "signup") &&
    result.session?.kind === "dev-session",
  );
}

function primeKakaoDevSession(
  result: Extract<KakaoCallbackResult, { action: "login" | "signup" }>,
) {
  persistedKakaoDevSession = {
...
function useParticipantFlow() {
  ...
  const profileReady =
    state.socialSessionStarted && hasRequiredDupr(state.profile);
  const applicationPolicy = {
    refundPolicy:
      state.application?.refundPolicy ?? "participantSelfCancelDisabled",
    supportChannel:
      state.application?.supportChannel ?? state.profile.supportChannel,
  } satisfies Pick<
    MockTournamentApplication,
    "refundPolicy" | "supportChannel"
  >;
  ...
  return useMemo(
    () => ({
      ...
      supportRefundPolicyCopy:
        state.supportCenter.policyCopy ||
        describeSupportRefundPolicyCopy(applicationPolicy),
    }),
...
export function startParticipantSession() {
  ...
  Promise.all([
    participantApi.getTournaments(),
    participantApi.getParticipantProfile(),
  ])
    .then(([tournaments, apiProfile]) => {
      patchParticipantState({
        featuredTournament:
          tournaments[0] ?? sandboxParticipantSession.featuredTournament,
        tournaments: tournaments.length
          ? tournaments
          : [sandboxParticipantSession.featuredTournament],
        profile: apiProfile,
...
function hydrateParticipantUtilityPages() {
  ...
  Promise.allSettled([
    participantApi.getSupportCenter(),
    participantApi.getNotifications(),
    participantApi.getMyPage(),
    participantApi.getGames(),
  ]).then(([supportResult, notificationResult, myPageResult, gamesResult]) => {
    const patch: Partial<ParticipantStoreState> = {
      routeStatus: { ...participantState.routeStatus },
    };
  ...
  participantApi
    ...
    .then((tournament) =>
      patchParticipantState({
        ...
        tournamentDivisions: tournament.divisions,
        apiMode: "api",
        routeStatus: {
          ...participantState.routeStatus,
          tournamentDetail: "ready",
        },
      }),
    )
    .catch(() =>
...
function saveDupr() {
  const fallbackProfile = saveSandboxDupr(
    participantState.profile,
    participantState.duprInput,
  );
  patchParticipantState({ profile: fallbackProfile, application: null });

  if (!participantApi.enabled) return;
  participantApi
    .updateParticipantProfile({ duprId: participantState.duprInput })
    .then((apiProfile) => {
      patchParticipantState({
        profile: apiProfile,
        duprInput: apiProfile.duprId ?? "",
...
function submitApplication() {
  ...
  participantApi
    ...
    .then((createdApplication) =>
      participantApi.getTournamentApplication(createdApplication.applicationId),
    )
    .then((apiApplication) => {
      patchParticipantState({ application: apiApplication, apiMode: "api" });
      if (participantApi.applicationBridgeOnly)
        refreshParticipantPaymentStatus();
      else hydrateParticipantUtilityPages();
    })
    .catch(() => patchParticipantState({ apiMode: "fallback" }));
}
...
function paymentStatusFromOrder(order: PaymentOrderResponse): PaymentRecord {
  return {
...
function upsertPaymentRecordFromOrder(order: PaymentOrderResponse) {
  const nextRecord = paymentStatusFromOrder(order);
  const paymentRecords = [
    nextRecord,
    ...participantState.paymentRecords.filter(
      (record) => record.paymentRecordId !== nextRecord.paymentRecordId,
    ),
  ];
  patchParticipantState({
...
function createPaymentAttemptId(applicationId: string) {
  return `mobile:${applicationId}:payment-order`;
}

function createPaymentOrder() {
  const applicationId = participantState.application?.applicationId;
  const amount =
    participantState.paymentRecords[0]?.amountKrw ??
    getSelectedDivision()?.entryFeeKrw;
  if (!participantApi.enabled || !applicationId || !amount) {
    patchParticipantState({
      paymentOrderSubmission: "failed",
      apiMode: participantApi.enabled ? participantState.apiMode : "fallback",
    });
    return;
  }

  patchParticipantState({ paymentOrderSubmission: "submitting" });
  participantApi
    .createPaymentOrder({
      applicationId,
      paymentMode: "card",
      amount,
      currency: "KRW",
      idempotencyKey: createPaymentAttemptId(applicationId),
    })
    .then((order) => {
      upsertPaymentRecordFromOrder(order);
      patchParticipantState({ paymentOrderSubmission: "submitted" });
      if (order.status === "paid") router.push("/payment-complete");
...
function pollActivePaymentStatus() {
  const activeOrderId = participantState.activePaymentOrder?.paymentRecordId;
  const paymentRecord = participantState.paymentRecords[0];
  const paymentRecordId = activeOrderId ?? paymentRecord?.paymentRecordId;
  if (
    !participantApi.enabled ||
    !paymentRecordId ||
    (!activeOrderId && paymentRecord?.paymentMode === "operatorManagedOffline")
  ) {
    refreshParticipantPaymentStatus();
    return;
  }
...
function requestRefundReview() {
  const paymentRecordId =
    participantState.activePaymentOrder?.paymentRecordId ??
    participantState.paymentRecords[0]?.paymentRecordId;
  if (!participantApi.enabled || !paymentRecordId) {
    submitSupportInquiry();
    patchParticipantState({ refundSubmission: "failed" });
    return;
  }

  patchParticipantState({ refundSubmission: "submitting" });
  participantApi
    .requestRefund(paymentRecordId, {
      reason: "참가자가 모바일에서 취소·환불 검토를 요청했습니다.",
    })
    .then((refundRequest) =>
      patchParticipantState({
        activeRefundRequest: refundRequest,
    ...
    .catch(() =>
      patchParticipantState({
        refundSubmission: "failed",
        apiMode: "fallback",
      }),
    );
}

function refreshRefundStatus() {
  const paymentRecordId =
    participantState.activePaymentOrder?.paymentRecordId ??
    participantState.paymentRecords[0]?.paymentRecordId;
  if (!participantApi.enabled || !paymentRecordId) return;
  patchParticipantState({ refundSubmission: "submitting" });
  participantApi
    .getRefundHistory(paymentRecordId)
    .then((response) =>
      patchParticipantState({
        activeRefundRequest: response.refundRequest,
        refundSubmission: "submitted",
        apiMode: "api",
      }),
    )
...
function submitSupportInquiry() {
  ...
  if (!participantApi.enabled) {
    patchParticipantState({
      ...
      supportCenter: {
        ...participantState.supportCenter,
        inquiries: [
          createFallbackSupportInquiry(),
          ...participantState.supportCenter.inquiries,
        ],
      },
    });
    return;
  ...
  participantApi
    ...
    .then((inquiry) =>
      patchParticipantState({
        ...
        supportCenter: {
          ...participantState.supportCenter,
          inquiries: [inquiry, ...participantState.supportCenter.inquiries],
        },
        apiMode: "api",
...
function HeaderBell() {
  return (
    <View testID="header-bell" style={styles.headerBell}>
  ...
  return (
    <View testID="court-preview" style={styles.courtPreview}>
      ...
      <View
        style={[
          styles.courtDot,
          styles.courtDotTopLeft,
          live && styles.courtDotLive,
        ]}
      />
      <View
        style={[
          styles.courtDot,
          styles.courtDotTopRight,
          live && styles.courtDotLive,
        ]}
      />
...
function Row({ left, right }: { left: string; right?: string }) {
  return (
    <View style={styles.infoRow}>
      <Text style={styles.bodyCopy}>{left}</Text>
...
function PageHero({
  ...
  return (
    <View testID={testID} style={styles.pageHero}>
...
function InfoCard({
  ...
  return (
    <View testID={testID} style={styles.infoCard}>
      {title ? <Text style={styles.sectionTitleSmall}>{title}</Text> : null}
      {children}
    </View>
  );
}
...
function InfoListItem({
  ...
  return (
    <View style={styles.infoListItem}>
      ...
      <Text style={muted ? styles.caption : styles.rowRight}>{value}</Text>
    </View>
  );
}
...
function StatusListCard({
  ...
  return (
    <View testID={testID} style={styles.statusListCard}>
      {badgeText ? <Text style={styles.badge}>{badgeText}</Text> : null}
      <Text style={styles.choiceTitle}>{title}</Text>
      <Text style={styles.bodyCopy}>{meta}</Text>
      {caption ? <Text style={styles.caption}>{caption}</Text> : null}
    </View>
  );
}
...
function routeStatusCopy(
  ...
  if (status === "fallback")
    return "일부 정보를 불러오지 못했습니다. 잠시 후 다시 확인해 주세요.";
  if (status === "ready") return undefined;
  return undefined;
}

function RouteStatusNotice({
  status,
}: {
  status?: NonNullable<
    ParticipantStoreState["routeStatus"][keyof ParticipantStoreState["routeStatus"]]
  ...
  return (
    <Text
      testID="participant-route-state"
      style={status === "fallback" ? styles.blockerText : styles.caption}
    >
      {copy}
    </Text>
  );
...
function paymentMethodCopy(paymentRecord?: PaymentRecord) {
  if (paymentRecord?.paymentMode === "operatorManagedOffline")
    return "운영자 오프라인 확인";
  return "운영자 결제 안내 대기";
}
...
const fallbackTournamentDivisions: TournamentDivision[] = [
  ...
  {
    divisionId: "local-mens",
    tournamentId: defaultTournamentId,
    name: "남자복식",
    skillLevel: "DUPR 3.5~4.5",
    teamType: "doubles",
    entryFeeKrw: 60000,
    capacityTeams: 64,
  },
...
function getAvailableDivisions(tournamentDivisions: TournamentDivision[]) {
  return tournamentDivisions.length
    ? tournamentDivisions
    : fallbackTournamentDivisions;
}

function getApplicationDivisionName(
  application: MockTournamentApplication | null,
  divisions: TournamentDivision[],
  defaultDivision?: TournamentDivision,
) {
  const selectedDivision = application?.divisionId
    ? divisions.find(
        (division) => division.divisionId === application.divisionId,
      )
    : undefined;
...
const bottomTabs = [
  ...
  },
  {
    label: "내 경기",
    route: "/games",
    testID: "bottom-tab-games",
    active: "games",
  },
  {
    label: "알림",
    route: "/notifications",
    testID: "bottom-tab-notifications",
    active: "notifications",
  },
  {
    label: "마이",
    route: "/mypage",
    testID: "bottom-tab-mypage",
    active: "mypage",
  },
] as const;
    ...
    <View testID="bottom-nav" style={styles.bottomNav}>
      {bottomTabs.map((tab) => (
        <Pressable
          ...
          style={[
            styles.navButton,
            active === tab.active && styles.navButtonActive,
          ]}
        >
          <Text
            style={[
              styles.bottomNavItem,
              active === tab.active && styles.bottomNavItemActive,
            ]}
          >
            {tab.label}
          </Text>
...
function LoginScreen({
  ...
  kakaoCallbackResult,
  kakaoContinuationStatus,
}: {
  participantApiClient?: ParticipantApiClient;
  socialLoginConfig?: SocialLoginConfig;
  kakaoCallbackResult?: KakaoCallbackResult | null;
  kakaoContinuationStatus?: { kind: "loading" | "error"; message: string };
}) {
  const initialized = useRef(false);
  if (!initialized.current) {
    resetParticipantFlow(participantApiClient);
    if (isKakaoDevSessionSuccess(kakaoCallbackResult))
      primeKakaoDevSession(kakaoCallbackResult);
  ...
  const socialLoginAvailabilityCopy =
    describeSocialLoginAvailability(socialLoginConfig);
  ...
  return (
    <View style={styles.loginScreen}>
      <View testID="login-artboard" style={styles.phoneFrame}>
        <View style={styles.loginMain}>
          <Logo />
          <Text testID="login-logo-text" style={styles.hiddenParityText}>
            Happickle
          </Text>
          <Text testID="login-subtitle" style={styles.tagline}>
            대한피클볼협회 공식 대회 플랫폼
          </Text>
          <View testID="login-illustration" style={styles.illWrap}>
            <Text style={styles.illIcon}>◌</Text>
            <Text style={styles.illHandle}>╲</Text>
          </View>
          <Pressable
            testID="kakao-login-button"
            accessibilityRole="button"
            accessibilityState={{ disabled: !kakaoOauthCheckable }}
            disabled={!kakaoOauthCheckable}
            onPress={startKakaoOAuth}
            style={[
              styles.btn,
              styles.kakaoButton,
              !kakaoOauthCheckable && styles.disabledSocialButton,
            ]}
          >
          ...
          {socialLoginAvailabilityCopy ? (
            <Text testID="social-login-pending-copy" style={styles.caption}>
              {socialLoginAvailabilityCopy}
            </Text>
          ) : null}
          {kakaoCallbackCopy ? (
            <Text
              testID="kakao-callback-result-copy"
              style={[styles.caption, styles.kakaoCallbackCopy]}
            >
              {kakaoCallbackCopy}
            </Text>
          ) : null}
          {kakaoContinuationStatus ? (
            <Text
              testID="kakao-auth-continuation-status"
              accessibilityLiveRegion="polite"
              style={[styles.caption, styles.kakaoCallbackCopy]}
            >
              {kakaoContinuationStatus.message}
            </Text>
          ) : null}
          {participantState.persistedKakaoDevSession ? (
            <Text
              testID="kakao-dev-session-persistence-copy"
              style={[styles.caption, styles.successCopy]}
            >
              카카오 로그인이 이 기기에서 보존되어 앱 재시작 후에도 이용을
              이어갈 수 있어요.
            </Text>
          ) : null}
          {kakaoCallbackResult &&
          "action" in kakaoCallbackResult &&
          kakaoCallbackResult.action === "additional_info_required" ? (
            <ActionButton
              testID="kakao-additional-info-button"
              label="추가 정보 입력하기"
              secondary
              onPress={() => {
                const next =
                  kakaoCallbackResult.next ?? "/auth/additional-info";
                router.push(
                  kakaoCallbackResult.continuationToken
                    ? `${next}?continuationToken=${encodeURIComponent(kakaoCallbackResult.continuationToken)}`
                    : next,
                );
              }}
            />
          ...
          <Pressable
            testID="sandbox-login-button"
            accessibilityRole="button"
            onPress={startSandboxPreview}
            style={[styles.btn, styles.previewButton]}
          >
            <Text style={styles.previewButtonText}>대회 둘러보기</Text>
          </Pressable>
          <Text testID="login-consent-copy" style={styles.hint}>
            처음이시면 자동으로 회원가입이 진행돼요
          </Text>
          <Pressable
            testID="signup-route-button"
            accessibilityRole="button"
            onPress={() => router.push("/signup")}
          >
            <Text style={styles.linkText}>회원가입 폼 미리보기</Text>
          </Pressable>
        </View>
      </View>
    </View>
...
function ParticipantRouteScaffold({
  active,
  children,
}: {
  active: string;
    ...
    <View style={styles.participantShell}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content}>
        <View style={styles.header}>
          <View>
            <Logo small />
            <Text style={styles.headerSubtitle}>대한피클볼협회 공식</Text>
          </View>
          <HeaderBell />
        </View>
        {children}
        <View testID="deferred-reference-screens" style={styles.sectionCard}>
          <Text style={styles.sectionLabel}>운영 예정 기능</Text>
          <Text style={styles.caption}>
            결제 · 환불 · 대진표 · 점수 입력 · 결과 확정은 운영자 안내에 따라
            순차적으로 제공됩니다.
          </Text>
        </View>
    ...
    <View style={styles.participantShell}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content}>
        ...
        <PageHero
          ...
          eyebrow="법적 고지"
          title={title}
          caption="개발·스테이징 검토용 문서입니다."
        />
        <Text
          testID={`${testID.replace("-screen", "")}-draft-notice`}
...
export function PrivacyPolicyScreen() {
  return (
    <PublicLegalScaffold
      ...
      title="개인정보처리방침"
      draftNotice="법무·개인정보 담당자가 검토 중인 초안이며, 게시된 방침이 아닙니다."
      body={legalDocumentWithCompanyInfo(privacyPolicyDraft)}
    />
  );
}
...
export function TermsScreen() {
  return (
    <PublicLegalScaffold
      testID="terms-screen"
      title="이용약관"
      draftNotice="법무 담당자가 검토 중인 초안이며, 게시된 약관이 아닙니다."
      body={legalDocumentWithCompanyInfo(termsDraft)}
    />
  );
}
...
function callbackResultFromParams(
  params: Record<string, string | string[] | undefined>,
): KakaoCallbackResult | undefined {
  const value = (key: string) =>
    typeof params[key] === "string" ? (params[key] as string) : undefined;
  const action = value("action");
  if (action === "additional_info_required")
    return {
      action,
      reason: value("reason"),
  ...
  if (action === "blocked" && value("reason"))
    return {
      action,
...
export default function Home({
  ...
  socialLoginConfig,
  kakaoCallbackResult,
  kakaoAuthClient = defaultKakaoAuthClient,
}: HomeProps = {}) {
  const params = useLocalSearchParams() as Record<
    string,
    string | string[] | undefined
  >;
  const callbackRedirect = kakaoAuthCallbackRedirectSchema.safeParse(
    Object.fromEntries(
      Object.entries(params).filter(([, value]) => typeof value === "string"),
    ),
  );
  const outcomeId =
    callbackRedirect.success && callbackRedirect.data.action === "auth_complete"
      ? callbackRedirect.data.outcomeId
      : undefined;
  const [kakaoContinuationStatus, setKakaoContinuationStatus] = useState<{
    kind: "loading" | "error";
    message: string;
  }>();
  useEffect(() => {
    if (!outcomeId) return;
    let active = true;
    setKakaoContinuationStatus({
      kind: "loading",
      message: "카카오 로그인을 완료하고 있어요.",
    });
    void kakaoAuthClient
      .continueAuth({ outcomeId })
      .then((result) => {
        if (!active) return;
        primeKakaoDevSession(result);
        router.push(
          result.action === "signup" ? "/signup-complete" : "/tournaments",
        );
      })
      .catch((error: unknown) => {
        if (!active) return;
        const expired =
          ...
          error.message === "AUTH_OUTCOME_NOT_PENDING";
        setKakaoContinuationStatus({
          kind: "error",
          message: expired
            ? "카카오 로그인 요청이 만료되었습니다. 다시 로그인해 주세요."
  ...
  }, [kakaoAuthClient, outcomeId]);
  return (
    <LoginScreen
      participantApiClient={participantApiClient}
      socialLoginConfig={socialLoginConfig}
      kakaoCallbackResult={
        kakaoCallbackResult ?? callbackResultFromParams(params)
      }
      kakaoContinuationStatus={kakaoContinuationStatus}
    />
  );
}
...
function describeKakaoAdditionalInfoError(error: unknown) {
  ...
  if (error instanceof Error && error.message === "DUPLICATE_PHONE")
    return "이미 가입된 연락처입니다. 1:1 문의로 확인해 주세요.";
  if (error instanceof Error && error.message === "ADDITIONAL_INFO_NOT_PENDING")
    return "추가 정보 입력 요청이 만료되었습니다. 카카오 로그인을 다시 시작해 주세요.";
  if (
    error instanceof Error &&
    error.message === "KAKAO_AUTH_API_NOT_CONFIGURED"
  )
    return "dev API 연결 설정을 확인해 주세요.";
  return "가입을 완료하지 못했습니다. 잠시 후 다시 시도해 주세요.";
}

export function KakaoAdditionalInfoScreen({
  continuationToken: continuationTokenProp,
  completeAdditionalInfo = defaultKakaoAuthClient.completeAdditionalInfo,
}: {
  continuationToken?: string;
  completeAdditionalInfo?: CompleteKakaoAdditionalInfo;
} = {}) {
  const params = useLocalSearchParams<{ continuationToken?: string }>();
  const continuationToken =
    continuationTokenProp ??
    (typeof params.continuationToken === "string"
  ...
  const submitAdditionalInfo = async () => {
    ...
    try {
      const result = await completeAdditionalInfo({
        continuationToken,
        email,
        displayName: name,
        ...(phone.trim() ? { phone } : {}),
  ...
  return (
    <ScrollView
      testID="kakao-additional-info-screen"
      style={styles.page}
      contentContainerStyle={styles.content}
    >
      <PageHero
        testID="kakao-additional-info-hero"
        eyebrow="카카오 가입"
        title="추가 정보를 확인해 주세요"
        caption="카카오 계정에서 전달되지 않은 필수 정보만 로컬 dev 화면에서 보완합니다."
      />
      <InfoCard testID="kakao-additional-info-fields" title="필수 정보">
        <TextInput
          testID="kakao-additional-email-input"
          accessibilityLabel="카카오 추가 이메일"
          autoCapitalize="none"
        ...
        />
        <TextInput
          testID="kakao-additional-name-input"
          accessibilityLabel="카카오 추가 이름"
        ...
        />
        <TextInput
          testID="kakao-additional-phone-input"
          accessibilityLabel="카카오 추가 연락처"
      ...
      <InfoCard testID="kakao-additional-info-notice" title="로컬 dev 안내">
        <Text style={styles.caption}>
          입력값은 dev API로 전송되며, Kakao REST/API 키나 토큰은 모바일에
          저장하지 않습니다.
        </Text>
      </InfoCard>
      {!continuationToken ? (
        <Text testID="kakao-additional-info-status" style={styles.blockerText}>
          카카오 로그인 정보가 없습니다. 로그인을 다시 시작해 주세요.
        </Text>
      ) : null}
      {submissionState === "submitting" ? (
        <Text testID="kakao-additional-info-status" style={styles.caption}>
          가입 정보를 확인하고 있어요.
        </Text>
      ) : null}
      {errorCopy ? (
      ...
      <ActionButton
        testID="kakao-additional-info-submit-button"
        label={
          submissionState === "submitting"
            ? "가입 처리 중…"
            : "카카오 가입 계속하기"
      ...
      />
      <ActionButton
        testID="kakao-additional-info-back-button"
        label="로그인으로 돌아가기"
        secondary
        onPress={() => router.push("/")}
      />
  ...
  return (
    ...
    >
      <PageHero
        testID="signup-hero"
        eyebrow="회원가입"
        title="기본 정보를 확인해 보세요"
        caption="M0B 화면을 반영한 로컬 미리보기입니다."
      />
      ...
      <InfoCard testID="signup-profile-fields" title="기본 정보">
        ...
        <View style={styles.actionRow}>
          <Text style={[styles.secondaryPill, styles.selectedPill]}>남성</Text>
          <Text style={styles.secondaryPill}>여성</Text>
        </View>
        <TextInput
          testID="signup-birth-input"
          accessibilityLabel="생년월일 또는 연령대"
          placeholder="생년월일 또는 연령대"
          placeholderTextColor={palette.muted}
          style={styles.input}
        />
        <TextInput
          testID="signup-dupr-input"
          accessibilityLabel="DUPR ID"
          placeholder="DUPR ID (선택)"
          placeholderTextColor={palette.muted}
          style={styles.input}
        />
        <Text style={styles.caption}>
          DUPR 프로필 스크린샷을 첨부하면 관리자 확인이 빨라져요
        </Text>
        <TextInput
          testID="signup-club-input"
          accessibilityLabel="소속 클럽"
          placeholder="소속 · 클럽 (선택)"
          placeholderTextColor={palette.muted}
          style={styles.input}
        />
      </InfoCard>
      <InfoCard testID="signup-agreements" title="약관 동의">
        <Text style={styles.bodyCopy}>
          ☑ [필수] 개인정보 수집 · 이용에 동의{`\n`}☑ [필수] 이용약관 동의{`\n`}
          ☐ [선택] 마케팅 정보 수신 동의
        </Text>
      </InfoCard>
      <Text testID="signup-local-notice" style={styles.blockerText}>
        이 미리보기에서 가입 정보는 전송되지 않습니다.
      </Text>
      <ActionButton
...
export function TournamentsScreen() {
  ...
  const visibleTournaments = tournaments.length
    ? tournaments
    : [featuredTournament];
  ...
  return (
    <ParticipantRouteScaffold active="tournaments">
      <View testID="explore-home" style={styles.heroCard}>
        <Text style={styles.heroTitle}>어떤 대회에 나가볼까요?</Text>
        <View style={styles.searchBox}>
          <Text style={styles.searchText}>⌕ 대회명으로 검색</Text>
        </View>
        <View style={styles.filterToolbar}>
          <Pressable
            testID="region-filter-button"
            accessibilityRole="button"
            onPress={() => setRegionSelectorOpen(true)}
          >
            <Text style={[styles.filterChip, styles.locationChip]}>
              ⌖ {region}⌄
            </Text>
          </Pressable>
          <Text style={styles.filterChip}>최신순⌄</Text>
        </View>
        <View style={styles.filterRow}>
          {["접수중", "접수마감", "종료"].map((chip) => (
            <Text
              key={chip}
              style={[
                styles.statusChip,
                chip === "접수중" && styles.activeChip,
              ]}
            >
              {chip}
            </Text>
        ...
        <View style={styles.cardTopRow}>
          <Text style={styles.sectionTitleSmall}>접수 중인 대회</Text>
          <Text testID="participant-api-mode" style={styles.countText}>
            총 {visibleTournaments.length}개
          </Text>
        </View>
      </View>
      {regionSelectorOpen ? (
        <InfoCard testID="region-selector-modal" title="지역 선택">
          {[
            ["seoul", "서울특별시"],
            ["gyeonggi", "경기도"],
            ["incheon", "인천광역시"],
            ...
            <Pressable
              key={key}
              testID={`region-option-${key}`}
              accessibilityRole="radio"
              accessibilityState={{ selected: pendingRegion === label }}
              onPress={() => setPendingRegion(label)}
      ...
      {visibleTournaments.map((tournament, index) => {
        ...
        return (
          <Pressable
            ...
            testID={
              index === 0 ? "mock-tournament-card" : "api-tournament-card"
            }
            accessibilityRole="button"
            onPress={() => router.push(tournamentPath)}
            ...
            <View style={styles.tournamentCardBody}>
              <View style={styles.cardTopRow}>
                <View style={styles.badgeRow}>
                  <Text style={styles.badge}>접수중</Text>
                  <Text style={styles.dDay}>
                    {tournamentDdayCopy(tournament.startsAt)}
                  </Text>
                </View>
                <Text style={styles.countText}>상세 보기</Text>
              </View>
              <Text style={styles.cardTitle}>{tournament.title}</Text>
              <Text style={styles.bodyCopy}>
                {formatTournamentDate(tournament.startsAt)}
              </Text>
              <Text style={styles.bodyCopy}>{tournament.location}</Text>
              <View style={styles.divisionRow}>
                <Text style={styles.divisionChip}>{tournament.division}</Text>
                <Text style={styles.divisionChip}>
                  {tournament.requiresDupr ? "DUPR 필수" : "DUPR 선택"}
                </Text>
              </View>
              <Text style={styles.caption}>결제 방식</Text>
              <Text style={styles.priceText}>
                {tournament.paymentMode === "operatorManagedOffline"
                  ? "운영자 오프라인 확인"
                  : tournament.paymentMode}
              </Text>
            </View>
          </Pressable>
        );
      })}
      <View testID="quick-actions" style={styles.sectionCard}>
        <Text style={styles.sectionTitle}>빠른 이동</Text>
        <ActionButton
          testID="go-support-button"
          label="1:1 문의 보기"
          secondary
          onPress={() => router.push("/support")}
        />
        <ActionButton
          testID="go-dupr-button"
          label="DUPR 등록하기"
          secondary
          onPress={() => router.push("/dupr-profile")}
        />
      </View>
...
export function TournamentDetailScreen({
  ...
  return (
    <ParticipantRouteScaffold active="tournaments">
      <PageHero
        testID="detail-layout-hero"
        eyebrow="대회 상세"
        title={featuredTournament.title}
        caption="대회 일정, 장소, 신청 부문을 확인하세요."
      >
        <CourtPreview live />
      </PageHero>
      <InfoCard testID="tournament-detail" title="대회 정보">
        ...
        <View style={styles.badgeRow}>
          <Text style={styles.badge}>접수중</Text>
          <Text style={styles.dDay}>
            {tournamentDdayCopy(featuredTournament.startsAt)}
          </Text>
        </View>
        <InfoListItem label="주최" value="대한피클볼협회" />
        <InfoListItem
          label="일정"
          value={formatTournamentDate(featuredTournament.startsAt)}
        />
        <InfoListItem label="장소" value={featuredTournament.location} />
        <InfoListItem
          label="신청 방식"
          value={`${featuredTournament.requiresDupr ? "DUPR 등록 후 " : ""}부문 확인 · 운영자 오프라인 결제 안내`}
        />
      ...
      <InfoCard title="신청 가능한 부문">
        {availableDivisions.map((division) => (
          <StatusListCard
            ...
            testID="division-option"
            title={`${division.name} · ${divisionTeamCopy(division.teamType)}`}
            meta={`${divisionEligibilityCopy(division)} · 마감 8/7`}
            caption={`정원 ${division.capacityTeams ?? 32}팀 · ${divisionFeeCopy(division)}`}
          />
        ))}
      </InfoCard>
      <InfoCard title="대회요강">
        <Text style={styles.caption}>
          · 경기방식: 예선 라운드로빈 후 본선 토너먼트
          {`
`}
...
`}
          ...
          {`
`}
          · 우천/불가항력 시 일정은 주최측 공지에 따름
        </Text>
      </InfoCard>
      <InfoCard title="환불 규정">
        <Text style={styles.caption}>
          대회 3일 전까지 100% 환불 · 3일 이내 환불 불가 · 주최 측 취소 시 전액
          환불됩니다. {policyCopy}
        </Text>
      </InfoCard>
      <ActionButton
        testID="detail-apply-button"
        label="참가 신청으로 이동"
        onPress={() => router.push(profileReady ? applyRoute : "/dupr-profile")}
      />
    </ParticipantRouteScaffold>
  );
}
...
export function DuprProfileScreen() {
  const { profile, duprInput, profileReady, featuredTournament } =
    useParticipantFlow();

  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="dupr-layout-hero"
        eyebrow="DUPR 정보 관리"
        title={
          profileReady
            ? "현재 DUPR 저장됨"
            : "참가 신청 전 DUPR 정보가 필요해요"
        }
        caption="DUPR ID 또는 프로필 링크를 저장하면 참가 신청을 이어갈 수 있어요."
      />
      <View testID="dupr-management" style={styles.duprCard}>
        <Text style={styles.bigDupr}>
          {hasRequiredDupr(profile) ? profile.duprId : "4.2"}
        </Text>
        <Text style={styles.statusStrong}>관리자 확인중</Text>
        <Text style={styles.bodyCopy}>
          DUPR 프로필 스크린샷 첨부 · 확인이 빨라져요 (JPG, PNG)
        </Text>
        <Text style={styles.caption}>
          DUPR 값은 자기신고 기준이며 대회 참가 시 참고용으로만 표시됩니다
        </Text>
      </View>
      <InfoCard title="DUPR 입력">
        <TextInput
          testID="dupr-input"
          accessibilityLabel="DUPR ID"
          onChangeText={setDuprInput}
          placeholder="DUPR ID 또는 프로필 링크"
          placeholderTextColor={palette.muted}
          value={duprInput}
          style={styles.input}
        />
        ...
        {hasRequiredDupr(profile) ? (
          <Text testID="saved-dupr" style={styles.statusStrong}>
            현재 DUPR {profile.duprId} · 관리자 확인중
          </Text>
        ) : null}
      </InfoCard>
      <ActionButton
        testID="dupr-continue-application"
        label="참가 신청 계속"
        onPress={() =>
          router.push(`/tournaments/${featuredTournament.tournamentId}/apply`)
        }
        disabled={!hasRequiredDupr(profile)}
      />
    </ParticipantRouteScaffold>
  );
}

export function TournamentApplicationScreen({
  tournamentId = defaultTournamentId,
}: {
  tournamentId?: string;
}) {
  ...
  const submittedDivisionName = getApplicationDivisionName(
    application,
    availableDivisions,
    selectedDivision,
  );
  ...
  return (
    <ParticipantRouteScaffold active="tournaments">
      <PageHero
        testID="application-layout-hero"
        eyebrow="참가 신청"
        title={featuredTournament.title}
        caption="부문, 참가자 정보, 약관을 확인한 뒤 신청을 접수하세요."
      />
      <InfoCard testID="application-form" title="신청 부문">
        <StatusListCard
          testID="application-division-summary"
          title={`기본 선택 부문 · ${selectedDivision.name}`}
          meta={`${divisionEligibilityCopy(selectedDivision)} · ${divisionTeamCopy(selectedDivision.teamType)}`}
          caption={divisionFeeCopy(selectedDivision)}
          badgeText="선택됨"
        />
        {availableDivisions.map((division) => (
          <Text key={division.divisionId} style={styles.caption}>
            · {division.name}: {divisionEligibilityCopy(division)} ·{" "}
            {divisionFeeCopy(division)}
          </Text>
        ))}
      </InfoCard>
      <InfoCard title="참가자 정보">
        <InfoListItem label="대표자" value={profile.displayName} />
        <InfoListItem
          label="DUPR"
          value={hasRequiredDupr(profile) ? `${profile.duprId}` : "미등록"}
        />
        <InfoListItem label="연락처" value="010-••••-5678" />
      </InfoCard>
      <InfoCard title="복식 파트너 초대">
        <Text style={styles.bodyCopy}>파트너 전화번호를 입력해 초대하세요</Text>
        <Text style={styles.linkText}>초대하기</Text>
        <Text style={styles.caption}>유효기간 72시간 · 링크 재발송 가능</Text>
      </InfoCard>
      <InfoCard title="약관 동의">
        <Text style={styles.caption}>
...
`}
          ...
          {`
`}
          신청 후 참가자 직접 취소와 환불은 1:1 문의로 운영자가 안내합니다.
          결제는 실시간 PG 없이 운영자 확인 후 오프라인으로 안내됩니다.
        </Text>
      </InfoCard>
      {!profileReady ? (
        <Text testID="application-blocker" style={styles.blockerText}>
          {REQUIRED_DUPR_ERROR}: DUPR 정보를 저장한 뒤 참가 신청을 진행할 수
          있어요.
        </Text>
      ) : null}
      <Pressable
      ...
      >
        <Text style={styles.primaryActionText}>
          {profileReady ? "참가 신청하기" : "DUPR 등록 후 신청 가능"}
        </Text>
      </Pressable>
      {application ? (
        <Text testID="application-submitted" style={styles.statusStrong}>
          {applicationSubmittedLabel(apiMode)} · 접수 부문{" "}
          {submittedDivisionName} · {describeApplicationPolicy(application)}
        </Text>
      ) : null}
      {application ? (
        <ActionButton
          testID="application-payment-button"
          label="결제 안내 확인"
          secondary
...
export function SupportScreen() {
  const {
    ...
    supportCenter,
    routeStatus,
    supportInquirySubmission,
  } = useParticipantFlow();

  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="support-layout-hero"
        eyebrow="고객센터"
        title="무엇을 도와드릴까요?"
        caption="환불·취소 요청과 대회 문의는 1:1 문의로 운영자가 확인합니다."
      />
      <Text testID="support-readiness-notice" style={styles.blockerText}>
        개발·스테이징에서는 접수 화면과 상태 안내를 확인할 수 있으며, 실제
        처리는 운영자가 확인합니다.
      </Text>
      <InfoCard testID="support-center" title="자주 묻는 질문">
        <RouteStatusNotice status={routeStatus.support} />
        <Text testID="support-copy" style={styles.bodyCopy}>
          {supportRefundPolicyCopy}
        </Text>
        <ActionButton
          testID="support-inquiry-submit"
          label={
            supportInquirySubmission === "submitting"
              ? "1:1 문의 접수 중"
              : "환불/취소 1:1 문의 접수"
          }
          onPress={submitSupportInquiry}
          disabled={supportInquirySubmission === "submitting"}
        />
        {supportInquirySubmission === "submitted" ? (
          <Text testID="support-inquiry-state" style={styles.statusStrong}>
            1:1 문의가 접수되었습니다. 운영자가 확인 후 안내합니다.
          </Text>
        ) : null}
        {supportInquirySubmission === "fallback" ? (
          <Text testID="support-inquiry-state" style={styles.blockerText}>
            문의 접수에 실패했습니다. 카카오톡 또는 이메일 1:1 문의로 접수해
            주세요.
          </Text>
        ) : null}
      </InfoCard>
      <InfoCard title="문의 내역">
        {supportCenter.inquiries.map((inquiry) => (
          <StatusListCard
            key={inquiry.inquiryId}
            title={inquiry.subject}
            meta={`상태 · ${inquiry.status}`}
            caption="운영자 확인 후 안내됩니다."
          />
        ))}
      </InfoCard>
      <InfoCard title="상태·정책 바로가기">
        <ActionButton
          testID="support-reservations-button"
          label="신청·결제·환불 상태"
          secondary
          onPress={() => router.push("/reservation-history")}
        />
        <ActionButton
          testID="support-terms-button"
          label="이용약관 초안"
          secondary
          onPress={() => router.push("/terms")}
        />
        <ActionButton
          testID="support-privacy-button"
          label="개인정보처리방침 초안"
          secondary
          onPress={() => router.push("/privacy-policy")}
        />
      </InfoCard>
      <InfoCard title="문의 채널">
        <View style={styles.actionRow}>
          <Text style={styles.secondaryPill}>카카오톡 1:1 문의</Text>
          <Text style={styles.secondaryPill}>이메일 1:1 문의</Text>
        </View>
        <Text style={styles.caption}>
          운영시간: {supportCenter.operatingHours}
          {`
`}
...
export function GamesScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="games">
      <PageHero
        testID="games-layout-hero"
        eyebrow="내 경기"
        title="신청한 경기 일정"
        caption="접수된 대회와 결제 상태를 한눈에 확인하세요."
      />
      <InfoCard testID="my-games-screen">
        <RouteStatusNotice status={routeStatus.games} />
        {participantGames.length ? (
          participantGames.map((game) => (
            <StatusListCard
              key={game.gameId}
              testID="participant-game-card"
              title={game.tournamentTitle}
              meta={`${formatTournamentDate(game.startsAt)} · ${game.location}`}
              caption={`${game.divisionName ?? "부문 확인 중"} · 신청 ${game.applicationStatus} · 결제 ${game.paymentStatus}${game.paymentAmountKrw ? ` · ${game.paymentAmountKrw.toLocaleString("ko-KR")}원` : ""}`}
              badgeText="예정"
            />
          ))
        ) : (
          <Text testID="games-empty" style={styles.caption}>
            아직 표시할 경기 일정이 없습니다. 대회 신청이 접수되면 여기에
            표시됩니다.
          </Text>
        )}
        <Text style={styles.caption}>
          점수 입력 · 결과 확정 · 대진표 운영은 운영자 안내 후 순차적으로
          제공됩니다.
        </Text>
      </InfoCard>
    </ParticipantRouteScaffold>
...
export function NotificationsScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="notifications">
      <PageHero
        testID="notifications-layout-hero"
        eyebrow="알림"
        title="중요한 안내를 모았어요"
        caption="대회 신청, 결제, 1:1 문의 답변을 놓치지 마세요."
      />
      <InfoCard testID="notifications-screen">
        <RouteStatusNotice status={routeStatus.notifications} />
        {notifications.length ? (
          notifications.map((notification) => (
            <StatusListCard
              ...
              caption={new Date(notification.createdAt).toLocaleDateString(
                "ko-KR",
              )}
            />
          ))
        ) : (
          <Text testID="notifications-empty" style={styles.caption}>
            아직 표시할 알림이 없습니다. 대회 신청, 1:1 문의 답변, 운영자 공지가
            생기면 여기에 표시됩니다.
          </Text>
        )}
      </InfoCard>
...
export function MyPageScreen() {
  ...
  const availableDivisions = getAvailableDivisions(tournamentDivisions);
  const paymentCopy = paymentRecords[0]
    ? `${paymentRecords[0].status} · ${paymentRecords[0].amountKrw.toLocaleString("ko-KR")}원 · 오프라인 운영자 확인`
    : "결제 내역 없음 · 오프라인 결제는 운영자 확인 대기";
  const recentApplicationCopy = application
    ? `최근 신청 · 접수 부문 ${getApplicationDivisionName(application, availableDivisions)}`
    : undefined;
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="mypage-layout-hero"
        eyebrow="마이"
        title={`${profile.displayName}님`}
        caption="내 신청, 결제, DUPR, 고객센터를 관리하세요."
      />
      <InfoCard testID="mypage-screen" title="프로필">
        ...
        <Text testID="mypage-payment-status" style={styles.bodyCopy}>
          {paymentCopy}
        </Text>
        {recentApplicationCopy ? (
          <Text testID="mypage-recent-application" style={styles.caption}>
            {recentApplicationCopy}
          </Text>
        ) : null}
      </InfoCard>
      <InfoCard title="빠른 메뉴">
        <ActionButton
          testID="mypage-reservations-button"
          label="예약 내역"
          secondary
          onPress={() => router.push("/reservation-history")}
        />
        <ActionButton
          testID="mypage-profile-edit-button"
          label="프로필 수정"
          secondary
          onPress={() => router.push("/profile-edit")}
        />
        <ActionButton
          testID="mypage-dupr-button"
          label="DUPR 정보 관리"
          secondary
          onPress={() => router.push("/dupr-profile")}
        />
        <ActionButton
          testID="mypage-notification-settings-button"
          label="알림 설정"
          secondary
          onPress={() => router.push("/notification-settings")}
        />
        <ActionButton
          testID="mypage-support-button"
          label="고객센터"
          secondary
          onPress={() => router.push("/support")}
        />
      ...
      <InfoCard title="계정">
        <Text style={styles.caption}>
          프로필 수정 · 내 경기 기록 · 결제 내역 · DUPR 정보 관리 · 알림 설정 ·
          고객센터 · 로그아웃
        </Text>
        <ActionButton
          testID="mypage-account-withdrawal-button"
          label="회원탈퇴 안내"
          secondary
...
export function ReservationHistoryScreen() {
  ...
  const reservationTitle = application
    ? featuredTournament.title
    : "예약 내역이 없습니다";
  const divisionName = application
    ? getApplicationDivisionName(application, availableDivisions)
    : "대회 신청 후 예약 내역이 표시됩니다";
  const paymentCopy = paymentRecord
    ? `${paymentRecord.amountKrw.toLocaleString("ko-KR")}원 · ${paymentRecord.status}`
    : "오프라인 결제는 운영자 확인 후 안내됩니다";

  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="reservation-history-hero"
        eyebrow="예약 내역"
        title="내 예약을 확인하세요"
        caption="신청한 대회와 결제 안내 상태를 한곳에서 확인합니다."
      />
      <InfoCard testID="reservation-history-screen" title={reservationTitle}>
        <RouteStatusNotice status={routeStatus.mypage} />
        <Text testID="reservation-status-notice" style={styles.blockerText}>
          신청 접수, 결제 확인, 취소·환불은 자동 완료되지 않으며 운영자 확인
          상태를 표시합니다.
        </Text>
        ...
        <Text style={styles.caption}>
          참가자 직접 취소와 환불은 1:1 문의로 운영자가 확인합니다.
        </Text>
      </InfoCard>
      <ActionButton
        testID="reservation-support-button"
        label="예약/환불 문의하기"
        secondary
        onPress={() => router.push("/support")}
      />
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        ...
        eyebrow="프로필 수정"
        title="기본 정보를 확인하세요"
        caption="이름, 소속, DUPR 정보는 참가 신청과 대회 운영에 사용됩니다."
      />
      <InfoCard testID="profile-edit-screen" title="기본 정보">
        ...
        />
        <Text style={styles.caption}>
          연락처와 본인 확인 정보 변경은 운영자 확인 후 반영됩니다.
        </Text>
      </InfoCard>
      <ActionButton
        testID="profile-edit-dupr-button"
        label="DUPR 정보 수정"
        secondary
        onPress={() => router.push("/dupr-profile")}
      />
...
function TerminalTournamentCard({
  testID,
  compact = false,
}: {
  testID?: string;
    ...
    <InfoCard testID={testID} title={featuredTournament.title}>
      ...
      <InfoListItem
        ...
        value={
          compact
            ? "8월 9일 (토) · 오전 9:00"
            : formatTournamentDate(featuredTournament.startsAt)
        }
      />
      <InfoListItem label="장소" value={featuredTournament.location} />
    </InfoCard>
...
export function PaymentScreen() {
  const {
    ...
    tournamentDivisions,
    activePaymentOrder,
    paymentOrderSubmission,
  } = useParticipantFlow();
  const paymentRecord = paymentRecords[0];
  const divisionName = getApplicationDivisionName(
    application,
    getAvailableDivisions(tournamentDivisions),
    getAvailableDivisions(tournamentDivisions)[0],
  );
  const liveStatus = activePaymentOrder?.status ?? paymentRecord?.status;
  const backendStatus =
    liveStatus === "orderCreated"
      ? "결제 주문 생성됨"
      : liveStatus === "pendingProvider"
        ? "결제 승인 대기 중"
        : liveStatus === "paid"
          ? "결제 완료 확인"
          : liveStatus === "failed"
            ...
            : liveStatus === "cancelled"
              ? "결제 취소됨"
              : paymentRecord?.status === "operatorReview"
                ? "운영자 확인 중"
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        ...
        eyebrow="결제"
        title="참가비 결제를 진행하세요"
        caption="백엔드 결제 주문 계약으로 생성하고 상태를 새로고침합니다. 외부 PG 실호출은 승인 전 사용하지 않습니다."
      />
      <InfoCard testID="payment-order-summary" title={featuredTournament.title}>
        <InfoListItem label="참가자" value={profile.displayName} />
        <InfoListItem
          label="신청 부문"
          value={application ? divisionName : "신청 접수 후 확인"}
        />
        <InfoListItem
          label="결제 금액"
          value={paymentAmountCopy(paymentRecord)}
        />
      </InfoCard>
      <InfoCard testID="payment-method" title="결제 상태">
        <Text style={styles.statusStrong}>백엔드 결제 주문</Text>
        <Text testID="payment-backend-status" style={styles.statusStrong}>
          {backendStatus}
        </Text>
        {activePaymentOrder?.providerOrderId ? (
          <Text
            testID="payment-provider-order-reference"
            style={styles.caption}
          >
            주문 참조 {activePaymentOrder.providerOrderId}
          </Text>
        ) : null}
        {paymentRecord ? (
          <Text testID="payment-record-reference" style={styles.caption}>
            결제 기록 {paymentRecord.paymentRecordId}
          </Text>
        ) : null}
        <Text style={styles.caption}>
          {paymentRecord?.operatorNote ??
            "결제 주문 생성 후 상태 새로고침으로 승인, 실패, 취소를 확인합니다."}
        </Text>
      </InfoCard>
      <InfoCard title="실패·지원 안내">
        <Text
          testID="payment-support-state"
          style={
            paymentOrderSubmission === "failed"
              ? styles.blockerText
              : styles.caption
        ...
        >
          {paymentOrderSubmission === "failed"
            ? "결제 주문을 생성하거나 조회하지 못했습니다. 다시 시도하거나 고객센터로 문의해 주세요."
            : "결제 실패 또는 지연 시 재시도와 고객센터 문의를 사용할 수 있습니다."}
        </Text>
      </InfoCard>
      <InfoCard title="환불 규정">
        <Text style={styles.caption}>
          환불은 결제 기록 기준으로 요청하며 운영자 검토 상태와 처리 이력을
          표시합니다.
        </Text>
      </InfoCard>
      <View testID="payment-readiness-notice">
        <RouteStatusNotice status={routeStatus.mypage} />
        <Text testID="payment-local-notice" style={styles.blockerText}>
          실시간 PG는 사용하지 않습니다. 백엔드 결제 계약과 운영자 확인 상태만
          표시하며 외부 결제 제공자 호출은 수행하지 않습니다.
        </Text>
      </View>
      <ActionButton
        testID="payment-create-order-button"
        label={
          paymentOrderSubmission === "submitting"
            ? "결제 주문 생성 중"
            : "결제 주문 생성"
        }
        onPress={createPaymentOrder}
        disabled={paymentOrderSubmission === "submitting"}
      />
      <ActionButton
        testID="payment-status-refresh"
        label={
          routeStatus.mypage === "loading"
            ? "결제 상태 확인 중"
            : "결제 상태 새로고침"
        }
        secondary
        onPress={pollActivePaymentStatus}
        disabled={routeStatus.mypage === "loading"}
      />
      <ActionButton
        testID="payment-support-button"
        label="결제 문의하기"
        secondary
        onPress={() => router.push("/support")}
      />
...
export function PaymentCompleteScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="payment-complete-hero"
        eyebrow="결제 상태"
        title="운영자 결제 확인 상태"
        caption="오프라인 결제 기록은 운영자 확인 후 반영됩니다."
      />
      <TerminalTournamentCard testID="payment-complete-screen" />
      <InfoCard title="운영자 확인 기록">
        <InfoListItem
          label="안내 금액"
          value={paymentAmountCopy(paymentRecord)}
        />
        <InfoListItem label="확인 시각" value="운영자 기록 시 표시" />
        <InfoListItem
          label="결제 방식"
          value={paymentMethodCopy(paymentRecord)}
        />
        <Text style={styles.statusStrong}>
          앱 내 결제 완료 또는 참가 확정을 의미하지 않습니다.
        </Text>
      </InfoCard>
      <ActionButton
        testID="payment-complete-games-button"
        label="내 경기 보기"
        onPress={() => router.push("/games")}
      />
      <ActionButton
        testID="payment-complete-home-button"
        label="홈으로"
        secondary
...
export function CancelConfirmScreen() {
  const { paymentRecords, activeRefundRequest, refundSubmission } =
    useParticipantFlow();
  const paymentRecord = paymentRecords[0];
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="cancel-confirm-hero"
        eyebrow="환불 요청"
        title="취소·환불 검토를 요청하세요"
        caption="백엔드 환불 요청 계약으로 접수하고 운영자 검토 상태를 표시합니다."
      />
      <TerminalTournamentCard testID="cancel-confirm-screen" />
      <InfoCard title="환불 검토 대상">
        <InfoListItem
          label="결제 기록 금액"
          value={paymentAmountCopy(paymentRecord)}
        />
        <InfoListItem
          ...
          value={paymentMethodCopy(paymentRecord)}
        />
        <Text testID="refund-review-status" style={styles.statusStrong}>
          {activeRefundRequest
            ? `환불 ${activeRefundRequest.status}`
            : refundSubmission === "failed"
              ? "환불 요청 실패"
              : "환불 요청 전"}
        </Text>
      ...
      <InfoCard title="환불 규정">
        <Text style={styles.caption}>
          취소·환불 가능 여부와 처리 일정은 백엔드 정책 평가와 운영자 확인 및
          검토 이력에 따라 표시됩니다.
        </Text>
      </InfoCard>
      <InfoCard title="요청 상태">
        <Text
          testID="refund-submission-state"
          style={
            refundSubmission === "failed" ? styles.blockerText : styles.caption
          }
        >
          {refundSubmission === "submitted"
            ? "환불 요청이 접수되어 운영자 검토 중입니다."
            : refundSubmission === "submitting"
              ? "환불 상태를 확인하고 있습니다."
              : refundSubmission === "failed"
                ? "환불 요청 또는 상태 확인에 실패했습니다. 고객센터 문의를 이용해 주세요."
                : "환불 요청은 결제 기록이 있는 경우에만 접수됩니다."}
        </Text>
      </InfoCard>
      <ActionButton
        testID="cancel-confirm-button"
        label={
          refundSubmission === "submitting" ? "환불 요청 중" : "환불 검토 요청"
        }
        onPress={requestRefundReview}
        disabled={refundSubmission === "submitting"}
      />
      <ActionButton
        testID="refund-status-refresh"
        label="환불 상태 새로고침"
        secondary
        onPress={refreshRefundStatus}
      />
      <ActionButton
        testID="refund-support-button"
        label="1:1 문의"
        secondary
        onPress={() => router.push("/support")}
      />
...
export function CancelCompleteScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="cancel-complete-hero"
        eyebrow="요청 상태"
        title="취소·환불 요청 접수"
        caption="운영자가 신청과 결제 기록을 확인한 뒤 처리 상태를 안내합니다."
      />
      <TerminalTournamentCard testID="cancel-complete-screen" />
      <InfoCard title="운영자 확인 대기">
        <InfoListItem
          label="확인 대상 금액"
          value={paymentAmountCopy(paymentRecords[0])}
        />
        <InfoListItem label="처리 일정" value="운영자 확인 후 안내" />
        <InfoListItem
          label="결제 방식"
          value={paymentMethodCopy(paymentRecords[0])}
        />
        <Text style={styles.blockerText}>
          취소 또는 환불이 완료된 상태가 아닙니다.
        </Text>
      </InfoCard>
      <ActionButton
        testID="cancel-complete-reservations-button"
        label="예약내역 보기"
        onPress={() => router.push("/reservation-history")}
      />
      <ActionButton
        testID="cancel-complete-home-button"
        label="홈으로"
        secondary
        onPress={() => router.push("/tournaments")}
      />
...
export function PaymentFailureScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="payment-failure-hero"
        eyebrow="결제 안내"
        title="결제 상태를 확인할 수 없습니다"
        caption="앱 내 결제 기능은 연결되지 않았습니다. 운영자 안내를 확인해 주세요."
      />
      <TerminalTournamentCard testID="payment-failure-screen" />
      <InfoCard title="결제 안내">
        <InfoListItem
          label="안내 금액"
          value={paymentAmountCopy(paymentRecords[0])}
        />
        <InfoListItem label="확인 상태" value="운영자 확인 대기" />
        <InfoListItem
          label="결제 방식"
          value={paymentMethodCopy(paymentRecords[0])}
        />
      ...
      <ActionButton
        testID="payment-failure-retry-button"
        label="결제 안내 다시 보기"
        onPress={() => router.push("/payment")}
      />
      <ActionButton
        testID="payment-failure-home-button"
        label="홈으로"
        secondary
        onPress={() => router.push("/tournaments")}
      />
...
export function InviteDetailScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="invite-detail-hero"
        eyebrow="초대장"
        title="피클볼 대회 파트너 초대장"
        caption={`${featuredTournament.title} · 남자복식 · 8월 9일 (토)`}
      />
      <InfoCard testID="invite-detail-screen" title={profile.displayName}>
        <Text style={styles.badge}>파트너 대기중</Text>
        <Text style={styles.bigDupr}>PICKLE-7X9K2</Text>
        <ActionButton
          testID="invite-kakao-button"
          label="카카오톡으로 초대하기"
          onPress={() => undefined}
        />
        <ActionButton
          ...
          onPress={() => undefined}
        />
        <Text style={styles.statusStrong}>대기중</Text>
        <Text style={styles.caption}>{inviteCountdownCopy()}</Text>
      </InfoCard>
      <ActionButton
        testID="invite-resend-button"
        label="링크 재발송 (최대 3회)"
        secondary
        onPress={() => undefined}
      />
...
export function PartnerAcceptScreen() {
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero
        testID="partner-accept-hero"
        eyebrow="파트너 수락"
        title="김민준님이 파트너로 초대했어요"
        caption="수락하면 대표자가 신청 및 결제를 진행합니다"
      />
      <TerminalTournamentCard testID="partner-accept-screen" compact />
      <InfoCard title="초대자 김민준 · DUPR 4.2">
        <Text style={styles.caption}>{inviteCountdownCopy()}</Text>
      </InfoCard>
      <ActionButton
      ...
      />
      <ActionButton
        testID="partner-decline-button"
        label="거절하기"
        secondary
        onPress={() => router.push("/partner-declined")}
      />
    </ParticipantRouteScaffold>
  );
}

function InviteTerminalScreen({
  testID,
  heroTestID,
  eyebrow,
  title,
...
export function InviteExpiredScreen() {
  return (
    <InviteTerminalScreen
      testID="invite-expired-screen"
      heroTestID="invite-expired-hero"
      eyebrow="초대 만료"
      title="초대 링크가 만료됐어요"
      caption="72시간 내 응답이 없었어요. 다시 초대하거나 취소해주세요"
    />
  );
}
...
export function PartnerDeclinedScreen() {
  return (
    <InviteTerminalScreen
      testID="partner-declined-screen"
      heroTestID="partner-declined-hero"
      eyebrow="초대 결과"
      title="이서연님이 초대를 거절했어요"
      caption="다른 파트너를 초대하거나 참가를 취소할 수 있어요"
    />
  );
}
    ...
    <ParticipantRouteScaffold active="games">
      <View testID="bracket-screen" style={styles.heroCard}>
        <PageHero
          ...
          eyebrow="토너먼트"
          title="대진표"
          caption="2026 협회장배 전국오픈 · 남자복식"
        />
        {bracketRounds.map((round) => (
          <View key={round.title} style={styles.sectionCard}>
            <Text style={styles.sectionTitleSmall}>{round.title}</Text>
            {round.matches.map(([date, teams, result]) => (
              <StatusListCard
                key={`${date}-${teams}`}
                title={teams}
                meta={date}
        ...
        <InfoCard testID="bracket-final-card" title="결승">
          <StatusListCard
            title="김민준/이서연 vs 강태양/이하늘"
            meta="8월 9일 · 센터코트"
            caption="11:8, 9:11, 11:6 · 우승"
            badgeText="우승"
          />
        </InfoCard>
      </View>
...
function MatchSummary() {
  return (
    <InfoCard title="8월 9일 (토) · 코트 3">
      <Text style={styles.cardTitle}>김민준/이서연 vs 박지훈/정수빈</Text>
    </InfoCard>
  );
}
  ...
  return (
    <ParticipantRouteScaffold active="games">
      <View testID="score-entry-screen" style={styles.heroCard}>
        <PageHero
          testID="score-entry-hero"
          eyebrow="경기 결과"
          title="점수 입력"
        />
        <MatchSummary />
        <InfoCard title="세트별 점수">
          {setScores.map(([set, ours, theirs]) => (
            <InfoListItem
              key={set}
              label={set}
              value={`${ours} : ${theirs} · 우리 팀 / 상대 팀`}
            />
          ))}
        </InfoCard>
        <Text style={styles.statusStrong}>
          입력한 결과는 상대팀 확인 후 확정됩니다
        </Text>
        <ActionButton
          testID="score-entry-submit-button"
          label="결과 제출하기"
          onPress={() => router.push("/result-confirm")}
        />
      </View>
    </ParticipantRouteScaffold>
  );
  ...
  return (
    <ParticipantRouteScaffold active="games">
      <View testID="result-confirm-screen" style={styles.heroCard}>
        <PageHero
          testID="result-confirm-hero"
          eyebrow="경기 결과"
          title="결과 확인"
        />
        <MatchSummary />
        <InfoCard title="제출된 결과">
          <Text style={styles.statusStrong}>승리: 김민준/이서연 (2:1)</Text>
          {setScores.map(([set, ours, theirs]) => (
            <InfoListItem key={set} label={set} value={`${ours} : ${theirs}`} />
          ))}
          <Text style={styles.caption}>김민준님이 8월 9일 14:32에 입력</Text>
        </InfoCard>
        <Text style={styles.caption}>
          내용이 맞으면 확인하고, 다르면 이의를 제기해주세요
        </Text>
        <ActionButton
          testID="result-confirm-button"
          label="내용이 맞아요, 확인하기"
          onPress={() => router.push("/final-results")}
        />
        <ActionButton
          testID="result-dispute-button"
          label="이의 제기하기"
          secondary
  ...
  return (
    <ParticipantRouteScaffold active="tournaments">
      <View testID="signup-complete-screen" style={styles.heroCard}>
        <PageHero
          testID="signup-complete-hero"
          eyebrow="가입 완료"
          title="회원가입이 완료됐어요"
          caption="Happickle와 함께 대회를 즐겨보세요"
        />
        <InfoCard title={`${profile.displayName}님, 반가워요!`}>
          <Text style={styles.bodyCopy}>
            DUPR {profile.duprId ?? "미등록"} · 송파피클볼클럽
          </Text>
        </InfoCard>
        <ActionButton
  ...
  return (
    <ParticipantRouteScaffold active="games">
      <View testID="dispute-screen" style={styles.heroCard}>
        ...
        <MatchSummary />
        <InfoCard title="제출된 결과">
          <Text style={styles.statusStrong}>11:7, 9:11, 11:8 (2:1 승)</Text>
        </InfoCard>
        <InfoCard title="이의 제기 사유">
          <TextInput
            testID="dispute-reason-input"
            accessibilityLabel="이의 제기 사유"
            multiline
            maxLength={200}
            placeholder={
              "어떤 부분이 다른지 구체적으로 적어주세요\n예: 2세트 점수가 9:11이 아니라 11:9입니다"
            }
            placeholderTextColor={palette.muted}
            style={[styles.input, styles.multilineInput]}
          />
          <Text style={styles.caption}>
            제출하면 주최자가 확인 후 결과를 수정·확정합니다
          </Text>
        </InfoCard>
        <ActionButton
          testID="dispute-submit-button"
          label="이의 제기 제출하기"
          onPress={() => router.push("/dispute-complete")}
        />
      </View>
    </ParticipantRouteScaffold>
  );
  ...
  return (
    <ParticipantRouteScaffold active="games">
      <View testID="dispute-complete-screen" style={styles.heroCard}>
        <PageHero
          testID="dispute-complete-hero"
          eyebrow="접수 완료"
          title="이의 제기가 접수됐어요"
          caption="주최측 확인 후 24시간 내 결과가 조정돼요"
        />
        <InfoCard title="2026 협회장배 전국오픈">
          <InfoListItem label="제출한 결과" value="11:7, 9:11, 11:8" />
          <InfoListItem
            label="이의 제기 사유"
            value="2세트 점수가 실제와 달라요"
          />
          <InfoListItem label="처리 예정" value="24시간 이내 확인" />
        </InfoCard>
        <ActionButton
          testID="dispute-complete-games-button"
          label="내 경기로 돌아가기"
          onPress={() => router.push("/games")}
        />
        <ActionButton
          testID="dispute-complete-home-button"
          label="홈으로"
          secondary
          onPress={() => router.push("/tournaments")}
        />
...
export function NotificationSettingsScreen() {
  const [preferences, setPreferences] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(
      notificationSettingRows.map(([key, , , enabled]) => [key, enabled]),
    ),
  );
  return (
    <ParticipantRouteScaffold active="mypage">
      <View testID="notification-settings-screen" style={styles.heroCard}>
        <PageHero
          testID="notification-settings-hero"
          eyebrow="마이"
          title="알림 설정"
          caption="이 기기에서 표시할 알림을 선택하세요. 설정은 현재 화면에만 임시로 유지됩니다."
        />
        <InfoCard title="알림 항목">
          {notificationSettingRows.map(([key, title, caption]) => (
            <Pressable
              key={key}
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <View testID="account-withdrawal-screen" style={styles.heroCard}>
        <PageHero
          testID="account-withdrawal-hero"
          eyebrow="회원탈퇴"
          title="회원탈퇴 안내"
          caption="계정 삭제는 본인 확인과 운영자 확인이 필요한 민감 작업입니다."
        />
        <InfoCard title="탈퇴 전 확인사항">
          <InfoListItem label="계정" value={profile.displayName} />
          <InfoListItem
            label="대회 신청"
            value="진행 중인 신청·결제·환불 내역 확인 필요"
          />
          <Text style={styles.caption}>
            참가 신청, 결제 확인, 환불, 경기 기록이 남아 있는 경우 고객센터 확인
            후 처리됩니다.
          </Text>
        </InfoCard>
        <InfoCard
          testID="account-withdrawal-disabled-state"
          title="현재 화면 동작"
        >
          <Text style={styles.blockerText}>
            이 미리보기에서는 회원탈퇴가 실행되지 않습니다.
          </Text>
          <Text style={styles.caption}>
            실제 계정 삭제, 개인정보 삭제, DB/API 호출은 별도 승인과 본인 확인
            후 처리해야 합니다.
          </Text>
        </InfoCard>
        <ActionButton
          testID="account-withdrawal-disabled-button"
          label="회원탈퇴 요청하기 (비활성)"
          onPress={() => undefined}
          disabled
        />
        <ActionButton
          testID="account-withdrawal-support-button"
          label="고객센터로 문의하기"
          secondary
          onPress={() => router.push("/support")}
        />
  ...
  return (
    <ParticipantRouteScaffold active="games">
      <View testID="final-results-screen" style={styles.heroCard}>
        <PageHero
          testID="final-results-hero"
          eyebrow="대회가 종료되었습니다"
          title="2026 협회장배 전국오픈"
          caption="남자복식 최종 결과"
        />
        <InfoCard title="최종 순위">
          {finalRanks.map(([rank, team, record]) => (
            <InfoListItem
              key={rank}
              label={`${rank}위`}
              value={`${team} · ${record}`}
        ...
        <InfoCard title="내 최종 순위">
          <Text style={styles.bigDupr}>5위</Text>
          <Text style={styles.bodyCopy}>김민준 · 이서연 · 8승 3패</Text>
        </InfoCard>
        <ActionButton
          testID="final-results-bracket-button"
          label="전체 대진표 보기"
          secondary
          onPress={() => router.push("/bracket")}
        />
        <ActionButton
          testID="final-results-home-button"
          label="홈으로"
          onPress={() => router.push("/tournaments")}
        />
      </View>
