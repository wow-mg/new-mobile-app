...
import { defaultKakaoAuthClient, type KakaoAuthClient } from '../auth/kakao-auth-client';
...
function Logo({ small = false }: { small?: boolean }) {
  return (
    <Pressable accessibilityRole="button" accessibilityLabel="메인으로 이동" onPress={() => router.push('/')}>
      <Image
...
function LoginScreen({ participantApiClient, socialLoginConfig = getSocialLoginConfig(), kakaoCallbackResult, kakaoContinuationStatus }: { participantApiClient?: ParticipantApiClient; socialLoginConfig?: SocialLoginConfig; kakaoCallbackResult?: KakaoCallbackResult | null; kakaoContinuationStatus?: { kind: 'loading' | 'error'; message: string } }) {
  ...
  const socialLoginAvailabilityCopy = describeSocialLoginAvailability(socialLoginConfig);
  ...
  return (
    <View style={styles.loginScreen}><View testID="login-artboard" style={styles.phoneFrame}><View style={styles.loginMain}>
      ...
      {socialLoginAvailabilityCopy ? <Text testID="social-login-pending-copy" style={styles.caption}>{socialLoginAvailabilityCopy}</Text> : null}
      ...
      {kakaoContinuationStatus ? <Text testID="kakao-auth-continuation-status" accessibilityLiveRegion="polite" style={[styles.caption, styles.kakaoCallbackCopy]}>{kakaoContinuationStatus.message}</Text> : null}
      {participantState.persistedKakaoDevSession ? <Text testID="kakao-dev-session-persistence-copy" style={[styles.caption, styles.successCopy]}>카카오 로그인이 이 기기에서 보존되어 앱 재시작 후에도 이용을 이어갈 수 있어요.</Text> : null}
...
function PublicLegalScaffold({ testID, title, body, draftNotice }: { testID: string; title: string; body: string; draftNotice: string }) {
  return (
    <View style={styles.participantShell}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content}>
        <View style={styles.header}><View><Logo small /><Text style={styles.headerSubtitle}>대한피클볼협회 공식</Text></View></View>
        <PageHero testID={`${testID}-hero`} eyebrow="법적 고지" title={title} caption="개발·스테이징 검토용 문서입니다." />
        <Text testID={`${testID.replace('-screen', '')}-draft-notice`} style={styles.blockerText}>{draftNotice}</Text>
...
export function PrivacyPolicyScreen() {
  return <PublicLegalScaffold testID="privacy-policy-screen" title="개인정보처리방침" draftNotice="법무·개인정보 담당자가 검토 중인 초안이며, 게시된 방침이 아닙니다." body={legalDocumentWithCompanyInfo(privacyPolicyDraft)} />;
}

export function TermsScreen() {
  return <PublicLegalScaffold testID="terms-screen" title="이용약관" draftNotice="법무 담당자가 검토 중인 초안이며, 게시된 약관이 아닙니다." body={legalDocumentWithCompanyInfo(termsDraft)} />;
}
...
function callbackResultFromParams(params: Record<string, string | string[] | undefined>): KakaoCallbackResult | undefined {
  ...
  if (action === 'additional_info_required') return { action, reason: value('reason'), continuationToken: value('continuationToken'), next: value('next') };
  if (action === 'blocked' && value('reason')) return { action, reason: value('reason') as string, message: value('message') };
  return undefined;
}

export default function Home({ participantApiClient, socialLoginConfig, kakaoCallbackResult, kakaoAuthClient = defaultKakaoAuthClient }: HomeProps = {}) {
  const params = useLocalSearchParams() as Record<string, string | string[] | undefined>;
  const callbackRedirect = kakaoAuthCallbackRedirectSchema.safeParse(Object.fromEntries(Object.entries(params).filter(([, value]) => typeof value === 'string')));
  const outcomeId = callbackRedirect.success && callbackRedirect.data.action === 'auth_complete' ? callbackRedirect.data.outcomeId : undefined;
  const [kakaoContinuationStatus, setKakaoContinuationStatus] = useState<{ kind: 'loading' | 'error'; message: string }>();
  useEffect(() => {
    if (!outcomeId) return;
    let active = true;
    setKakaoContinuationStatus({ kind: 'loading', message: '카카오 로그인을 완료하고 있어요.' });
    void kakaoAuthClient.continueAuth({ outcomeId })
      .then((result) => {
        if (!active) return;
        primeKakaoDevSession(result);
        router.push(result.action === 'signup' ? '/signup-complete' : '/tournaments');
      })
      .catch((error: unknown) => {
        if (!active) return;
        const expired = error instanceof Error && error.message === 'AUTH_OUTCOME_NOT_PENDING';
        setKakaoContinuationStatus({ kind: 'error', message: expired ? '카카오 로그인 요청이 만료되었습니다. 다시 로그인해 주세요.' : '카카오 로그인을 완료하지 못했습니다. 잠시 후 다시 시도해 주세요.' });
      });
    return () => { active = false; };
  }, [kakaoAuthClient, outcomeId]);
  return <LoginScreen participantApiClient={participantApiClient} socialLoginConfig={socialLoginConfig} kakaoCallbackResult={kakaoCallbackResult ?? callbackResultFromParams(params)} kakaoContinuationStatus={kakaoContinuationStatus} />;
}
...
export function SupportScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      ...
      <Text testID="support-readiness-notice" style={styles.blockerText}>개발·스테이징에서는 접수 화면과 상태 안내를 확인할 수 있으며, 실제 처리는 운영자가 확인합니다.</Text>
      ...
      <InfoCard title="상태·정책 바로가기"><ActionButton testID="support-reservations-button" label="신청·결제·환불 상태" secondary onPress={() => router.push('/reservation-history')} /><ActionButton testID="support-terms-button" label="이용약관 초안" secondary onPress={() => router.push('/terms')} /><ActionButton testID="support-privacy-button" label="개인정보처리방침 초안" secondary onPress={() => router.push('/privacy-policy')} /></InfoCard>
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      ...
      <InfoCard testID="reservation-history-screen" title={reservationTitle}>
        ...
        <Text testID="reservation-status-notice" style={styles.blockerText}>신청 접수, 결제 확인, 취소·환불은 자동 완료되지 않으며 운영자 확인 상태를 표시합니다.</Text>
...
export function PaymentCompleteScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero testID="payment-complete-hero" eyebrow="결제 상태" title="운영자 결제 확인 상태" caption="오프라인 결제 기록은 운영자 확인 후 반영됩니다." />
      <TerminalTournamentCard testID="payment-complete-screen" />
      <InfoCard title="운영자 확인 기록">
        <InfoListItem label="안내 금액" value={paymentAmountCopy(paymentRecord)} />
        <InfoListItem label="확인 시각" value="운영자 기록 시 표시" />
        <InfoListItem label="결제 방식" value={paymentMethodCopy(paymentRecord)} />
        <Text style={styles.statusStrong}>앱 내 결제 완료 또는 참가 확정을 의미하지 않습니다.</Text>
      </InfoCard>
...
export function CancelConfirmScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      ...
      <InfoCard title="운영자 환불 검토 대상">
        <InfoListItem label="결제 기록 금액" value={paymentAmountCopy(paymentRecord)} />
        <InfoListItem label="결제수단" value={paymentMethodCopy(paymentRecord)} />
        <Text style={styles.statusStrong}>환불 가능 여부 운영자 확인 대기</Text>
        <Text style={styles.priceText}>{paymentAmountCopy(paymentRecord)}</Text>
      </InfoCard>
      <InfoCard title="환불 규정"><Text style={styles.caption}>취소·환불 가능 여부와 처리 일정은 운영자가 신청 및 결제 기록을 확인한 뒤 안내합니다. 이 화면의 안내는 환불 확정 또는 처리 완료를 의미하지 않습니다.</Text></InfoCard>
      <InfoCard title="취소 사유 (선택)"><View style={styles.actionRow}>{['실수 신청', '일정 변경', '개인 사정', '기타'].map((reason) => <Text key={reason} style={styles.secondaryPill}>{reason}</Text>)}</View><Text style={styles.caption}>문의 접수 후 운영자가 확인하며, 이 화면에서 취소 또는 환불이 확정되지 않습니다.</Text></InfoCard>
      <ActionButton testID="cancel-confirm-button" label="취소·환불 1:1 문의" onPress={() => router.push('/support')} />
    </ParticipantRouteScaffold>
  );
}
...
export function CancelCompleteScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero testID="cancel-complete-hero" eyebrow="요청 상태" title="취소·환불 요청 접수" caption="운영자가 신청과 결제 기록을 확인한 뒤 처리 상태를 안내합니다." />
      <TerminalTournamentCard testID="cancel-complete-screen" />
      <InfoCard title="운영자 확인 대기"><InfoListItem label="확인 대상 금액" value={paymentAmountCopy(paymentRecords[0])} /><InfoListItem label="처리 일정" value="운영자 확인 후 안내" /><InfoListItem label="결제 방식" value={paymentMethodCopy(paymentRecords[0])} /><Text style={styles.blockerText}>취소 또는 환불이 완료된 상태가 아닙니다.</Text></InfoCard>
...
export function PaymentFailureScreen() {
  ...
  return (
    <ParticipantRouteScaffold active="mypage">
      <PageHero testID="payment-failure-hero" eyebrow="결제 안내" title="결제 상태를 확인할 수 없습니다" caption="앱 내 결제 기능은 연결되지 않았습니다. 운영자 안내를 확인해 주세요." />
      <TerminalTournamentCard testID="payment-failure-screen" />
      <InfoCard title="결제 안내"><InfoListItem label="안내 금액" value={paymentAmountCopy(paymentRecords[0])} /><InfoListItem label="확인 상태" value="운영자 확인 대기" /><InfoListItem label="결제 방식" value={paymentMethodCopy(paymentRecords[0])} /></InfoCard>
      <ActionButton testID="payment-failure-retry-button" label="결제 안내 다시 보기" onPress={() => router.push('/payment')} />
...
export function PartnerAcceptScreen() {
  return (
    <ParticipantRouteScaffold active="mypage">
      ...
      <ActionButton testID="partner-accept-button" label="초대 수락 후 결제 안내" onPress={() => router.push('/payment')} />
