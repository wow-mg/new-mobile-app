...
} from "../src/participant/api-client";
import {
...
describe("participant API client", () => {
  ...
  it("stays disabled without bearer auth so the app can use mock fallback", async () => {
    ...
    await expect(client.getTournaments()).rejects.toThrow(
      "PARTICIPANT_API_NOT_CONFIGURED",
    );
  });
  ...
  it("never reads a bearer credential from public mobile environment config", () => {
    ...
    process.env.EXPO_PUBLIC_PARTICIPANT_API_BEARER_TOKEN =
      "public-token-shaped-value";
  ...
  it("calls authenticated participant MVP endpoints and parses responses", async () => {
    const fetchImpl = jest.fn(async (url: string, init?: RequestInit) => {
      ...
      if (url.endsWith("/api/tournaments"))
        return jsonResponse({ tournaments: [tournament] });
      if (url.endsWith("/api/tournaments/tournament_api_001"))
        return jsonResponse(tournamentDetail);
      if (url.endsWith("/api/participant/profile") && init?.method === "GET")
        return jsonResponse(profile);
      if (url.endsWith("/api/participant/support") && init?.method === "GET")
        return jsonResponse({
          policyCopy:
            "참가자 직접 취소 불가 · 1:1 문의. Participant self-cancel/refund is not available in MVP.",
          contactEmail: "support@happickle.kr",
          operatingHours: "평일 10:00 ~ 18:00",
          inquiries: [],
        });
      if (
        url.endsWith("/api/participant/support/inquiries") &&
        init?.method === "POST"
      )
        return jsonResponse({
          inquiryId: "inquiry_api_002",
          participantId: profile.participantId,
      ...
      if (url.endsWith("/api/participant/notifications"))
        return jsonResponse({
          notifications: [
            {
              notificationId: "notification_api_001",
              participantId: profile.participantId,
              type: "support",
              title: "API 알림",
              body: "API 본문",
      ...
      if (url.endsWith("/api/participant/mypage"))
        return jsonResponse({
          ...
          paymentRecords: [
            {
              paymentRecordId: "payment_api_001",
      ...
      if (url.endsWith("/api/participant/games"))
        return jsonResponse({
          games: [
            {
              gameId: "game_api_001",
      ...
      if (url.endsWith("/api/participant/profile") && init?.method === "PATCH")
        return jsonResponse({ ...profile, duprId: body.duprId });
      if (
        url.endsWith("/api/tournament-applications") &&
        init?.method === "POST"
      )
        return jsonResponse({ ...application, ...body });
      if (
        url.endsWith("/api/tournament-applications/application_api_001") &&
        init?.method === "GET"
      )
      ...
      if (
        ...
        return jsonResponse(
          {
            error:
              participantApplicationErrorCodeSchema.enum
    ...
    await expect(
      client.getTournament(tournament.tournamentId),
    ).resolves.toEqual(tournamentDetail);
    await expect(client.getParticipantProfile()).resolves.toEqual(profile);
    await expect(client.getSupportCenter()).resolves.toMatchObject({
      contactEmail: "support@happickle.kr",
    });
    await expect(
      client.createSupportInquiry({
        category: "refund",
    ...
    await expect(client.getNotifications()).resolves.toMatchObject({
      notifications: [expect.objectContaining({ title: "API 알림" })],
    });
    await expect(client.getMyPage()).resolves.toMatchObject({
      paymentRecords: [expect.objectContaining({ amountKrw: 60000 })],
    });
    await expect(client.getGames()).resolves.toEqual([
      expect.objectContaining({
        tournamentTitle: "API Open",
        dataSource: "db",
      }),
    ]);
    await expect(
      client.updateParticipantProfile({ duprId: "DUPR-777" }),
    ).resolves.toMatchObject({ duprId: "DUPR-777" });
    await expect(
      client.createTournamentApplication({
        tournamentId: tournament.tournamentId,
        participantId: profile.participantId,
        duprId: profile.duprId,
        divisionId: "division_api_001",
    ...
    await expect(
      client.getTournamentApplication(application.applicationId),
    ).resolves.toEqual(application);
    await expect(
      client.requestParticipantSelfCancel(application.applicationId),
    ).rejects.toThrow(
      participantApplicationErrorCodeSchema.enum
        .PARTICIPANT_SELF_CANCEL_DISABLED,
    );

    const apiApplication = await client.getTournamentApplication(
      application.applicationId,
    );
    expect(describeApplicationPolicy(apiApplication)).toBe(
      "참가자 직접 취소 불가 · 1:1 문의",
    );
    expect(describeSupportRefundPolicyCopy(apiApplication)).toContain(
      "참가자 직접 취소/환불은 1:1 문의로 운영자가 확인합니다.",
    );

    expect(fetchImpl).toHaveBeenCalledWith(
      expect.stringMatching(/^https:\/\/api\.example\.invalid\/api\//),
      expect.objectContaining({
  ...
  it("calls payment order, status polling, and refund review contracts with zod parsing", async () => {
    const paymentOrder = {
    ...
    const fetchImpl = jest.fn(async (url: string, init?: RequestInit) => {
      if (url.endsWith("/api/payments/orders") && init?.method === "POST")
        return jsonResponse(paymentOrder, { status: 201 });
      if (
        url.endsWith("/api/payments/payment_api_card_001") &&
        init?.method === "GET"
      )
        return jsonResponse(paidOrder);
      if (
        url.endsWith("/api/payments/payment_api_card_001/refunds") &&
        init?.method === "POST"
      )
    ...
    await expect(
      client.getPaymentStatus("payment_api_card_001"),
    ).resolves.toMatchObject({ status: "paid" });
    await expect(
      client.requestRefund("payment_api_card_001", {
        reason: "참가자가 모바일에서 취소·환불 검토를 요청했습니다.",
      }),
    ).resolves.toMatchObject({ status: "operatorReview" });
    await expect(
      client.getRefundHistory("payment_api_card_001"),
    ).resolves.toMatchObject({ refundRequest: { status: "operatorReview" } });
  });

  it("surfaces API application policy errors before falling back to generic HTTP status", async () => {
    const duprRequiredFetch = jest.fn(async () =>
      jsonResponse(
        {
          error:
            participantApplicationErrorCodeSchema.enum.DUPR_PROFILE_REQUIRED,
        },
        { status: 400 },
      ),
    ) as unknown as typeof fetch;
    const duprRequiredClient = createParticipantApiClient({
      baseUrl: "https://api.example.invalid",
      bearerToken: "test-token",
      fetchImpl: duprRequiredFetch,
    });

    await expect(
      duprRequiredClient.createTournamentApplication({
        tournamentId: tournament.tournamentId,
        participantId: profile.participantId,
      }),
    ).rejects.toThrow(
    ...
    const unavailableClient = createParticipantApiClient({
      baseUrl: "https://api.example.invalid",
      bearerToken: "test-token",
      fetchImpl: unavailableFetch,
    });

    await expect(unavailableClient.getParticipantProfile()).rejects.toThrow(
      "PARTICIPANT_API_HTTP_503",
    );

    const notFoundFetch = jest.fn(async () =>
      jsonResponse(
        { error: participantApiErrorCodeSchema.enum.TOURNAMENT_NOT_FOUND },
        { status: 404 },
      ),
    ) as unknown as typeof fetch;
    const notFoundClient = createParticipantApiClient({
      baseUrl: "https://api.example.invalid",
      bearerToken: "test-token",
      fetchImpl: notFoundFetch,
    });

    await expect(notFoundClient.getTournament("missing")).rejects.toThrow(
      participantApiErrorCodeSchema.enum.TOURNAMENT_NOT_FOUND,
    );
  });
  ...
  it("falls back to HTTP status when API failure has no application error code", async () => {
    ...
    await expect(client.getParticipantProfile()).rejects.toThrow(
      "PARTICIPANT_API_HTTP_503",
    );
  });
});
